...
from .models import Provider, PipelineTemplate, JobTemplate, PipelineInstance, JobInstance
...
class ProviderAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'provider_type', 'api_base_url')
        }),
...
class JobTemplateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('pipeline_template', 'name', 'description', 'provider')
        }),
...
class PipelineInstanceAdmin(admin.ModelAdmin):
    list_display = ('id', 'pipeline_template', 'status', 'created_at', 'started_at', 'finished_at')
    list_filter = ('status', 'pipeline_template')
    search_fields = ('pipeline_template__name',)
    readonly_fields = ('created_at', 'started_at', 'finished_at')
    fieldsets = (
        (None, {
            'fields': ('pipeline_template', 'status')
        }),
        ('Parameters', {
...
class JobInstanceAdmin(admin.ModelAdmin):
    list_display = ('id', 'job_template', 'pipeline_instance', 'status', 'created_at', 'started_at', 'finished_at')
    ...
    readonly_fields = ('callback_id', 'created_at', 'queued_at', 'started_at', 'finished_at')
    fieldsets = (
        (None, {
            'fields': ('pipeline_instance', 'job_template', 'status', 'callback_id')
        }),
        ('Execution Details', {
            'fields': ('execution_parameters', 'external_job_id', 'results', 'logs_url'),
        }),
        ('Timestamps', {
