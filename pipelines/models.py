...
class Provider(models.Model):
    ...
    name = models.CharField(
        max_length=100, unique=True, help_text="e.g., GitHub Actions, GitLab CI"
    )
    provider_type = models.CharField(
        max_length=100, help_text="Type of provider (arbitrary name)"
    )
...
class JobTemplate(models.Model):
    ...
    pipeline_template = models.ForeignKey(
        PipelineTemplate, on_delete=models.CASCADE, related_name="job_templates"
    )
    name = models.CharField(max_length=200)
    description = models.TextField(blank=True)
    provider = models.ForeignKey(
        Provider, on_delete=models.PROTECT, related_name="job_templates"
    )
...
class JobInstance(models.Model):
    ...
    pipeline_instance = models.ForeignKey(
        PipelineInstance, on_delete=models.CASCADE, related_name="job_instances"
    )
    job_template = models.ForeignKey(
        JobTemplate, on_delete=models.PROTECT, related_name="instances"
    )
    status = models.CharField(
        max_length=20, choices=Status.choices, default=Status.PENDING, db_index=True
    )
